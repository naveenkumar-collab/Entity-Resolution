...
def run_threshold_tuning(n_val_eval: int = 2000):
    ...
    model = EntityMatchingModel.load("D:/evalution/student_resource/model_lgb.joblib")
    
    print(f"Loading {n_val_eval:,} validation entities...")
    val_s1 = pd.read_csv("D:/evalution/student_resource/val_source1_30k.tsv", sep="\t").head(n_val_eval)
    val_gt = pd.read_csv("D:/evalution/student_resource/val_ground_truth_30k.tsv", sep="\t").head(n_val_eval)
    ...
    s2_df = pd.read_csv("D:/evalution/student_resource/dataset/train/train_source2.tsv", sep="\t", nrows=250000)
    s3_df = pd.read_csv("D:/evalution/student_resource/dataset/train/train_source3.tsv", sep="\t", nrows=250000)
    ...
    pd.Series(opt_info).to_json("D:/evalution/student_resource/optimal_thresholds.json")
    print("Saved optimal thresholds to optimal_thresholds.json", flush=True)
