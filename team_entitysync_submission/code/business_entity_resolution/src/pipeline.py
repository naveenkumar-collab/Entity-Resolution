...
def run_pipeline(
    ...
    if os.path.exists(model_path):
        ...
        # Fallback to local student_resource location
        fallback_path = os.path.join("D:/evalution/student_resource", os.path.basename(model_path))
        print(f"Loading model from {fallback_path}...", flush=True)
        model = EntityMatchingModel.load(fallback_path)
