...
def run_benchmark():
    ...
    val_s1 = pd.read_csv(str(val_s1_path), sep="\t").head(5000)
    val_gt = pd.read_csv(str(val_gt_path), sep="\t").head(5000)
    ...
    s2_chunks = pd.read_csv(str(s2_path), sep="\t", nrows=200000)
    s3_chunks = pd.read_csv(str(s3_path), sep="\t", nrows=200000)
    ...
    if needed_missing:
        ...
        s2_extra = []
        with open(str(s2_path), "r", encoding="utf-8") as f:
        ...
        s3_extra = []
        with open(str(s3_path), "r", encoding="utf-8") as f:
            header = next(f).strip().split("\t")
            for line in f:
                parts = line.strip().split("\t")
                if parts[0] in needed_missing:
